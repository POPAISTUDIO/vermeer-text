...
## 5. Feuille de route — sept chantiers, dans l'ordre des dépendances

1. **Hygiène** *(aujourd'hui, ~1h)* — régénérer les 3 tokens exposés (méthode tuyau, les deux repos) + memory edit « prod sur v0.10.23 ».
2. **Ruleset gitops-prod** *(15 min, autonome)* — approvals 0 + required status checks + bypass actor tracé. Clôt l'incident bypass, débloque l'agent MEP.
3. **GOVERNANCE.md + registre des identités** *(fil dédié, prompt livré)* — la source de vérité, incluant les trois amendements du §3.
4. **Hermes greffier + triage désignateur** *(fil dédié)* — amendement du mandat Hermes (`gh issue create` sur ordre + réflexe de cadrage) et amendement du workflow qa-triage (pose de label automatique + les 6 garde-fous du §2.1). Active « une phrase → une fonctionnalité » et « un rouge QA → une PR ».
5. **Vérification prod** *(fil dédié)* — compte de service de test (supprime aussi la fragilité QA_STORAGE_STATE), puis Point 0 automatique + smoke en workflow GitHub.
6. **Migration des crons Hermes** *(fil dédié, parallélisable avec 5)* — scan de nuit en cron GitHub, puis canary prod et check de drift.
7. **Agent MEP** *(fil dédié — dépend de 2, 3 et 5)* — workflow de préparation tag + digest + checks, token branches-only, Dossier MEP.
8. **Agent comm'** *(fil dédié, dernier — le déclencheur idéal est le rapport MEP du chantier 7, mais une version dégradée sur workflow_dispatch via Hermes est possible dès le chantier 4)* — rapatriement du deck dans `docs/comm/`, workflow delta + PR deck/changelog + Dossier Releases.

Le backlog existant vit en parallèle et ne dépend de rien ci-dessus : vague 2 de la QA (voir §6), config auto-reload (un jour calme), relance Loki staging auprès d'Oussama (GATEWAY_TIMEOUT + rétention — périmètre infra), nettoyages post-MEP, WEB-01/#114, dette CI #86.
