...
| | |
|---|---|
| **Identité** | Session Claude Code locale sur le Mac, **hors GitHub Actions**. Capteur à la voix, diagnostics, sessions sur ordre — et, depuis le 30/07/2026, **greffier d'issues sur ordre explicite**. Rien ne migre en CI côté interactif |
| **Support du mandat** | **`~/.hermes/skills/autonomous-ai-agents/vermeer-operations/SKILL.md`**, section `Role & Mandate` — OBSERVED 30/07/2026. **Il n'existe aucun fichier « mandat »** : `vermeer-controle` est un *titre de session* Hermes, pas un document, et le mandat était jusque-là collé en session. Ce SKILL.md est le seul support qui se recharge de lui-même à chaque session, donc le seul qui gouverne réellement — c'est lui qu'on amende (§1 règle 6, [issue 135](https://github.com/POPAISTUDIO/vermeer-text/issues/135)). Les deux autres couches : `~/.hermes/SOUL.md` (constitution transverse, dont l'article anti-délégation) et `~/.hermes/memories/MEMORY.md` (mémoire de travail) |
