...
> **Écart relevé.** Le chantier 1 (hygiène, 29/07/2026) est réputé avoir régénéré trois jetons exposés. Les dates ci-dessus n'en portent la trace que pour `CLAUDE_CODE_OAUTH_TOKEN` (les deux dépôts, `2026-07-29T14:56`). `GITOPS_PUSH_TOKEN` et `VERMEER_SECRETS_TOKEN` portent des dates du **26/07**, antérieures au chantier. `QA_STORAGE_STATE` (29/07 à 05h59) est mis à jour **automatiquement à chaque run de QA** (rotation de session), sa date ne dit donc rien d'une rotation de jeton. Deux lectures possibles — les deux jetons gitops/secrets ont été régénérés le 26/07 lors d'une passe antérieure, **ou** ils n'ont pas été régénérés le 29/07 : **UNKNOWN**, à trancher par la personne qui a conduit le chantier 1. À vérifier à la revue mensuelle ([§7, point 1](GOVERNANCE.md#revue-mensuelle--30-minutes-checklist)).
...
| **Permis** | Résoudre le tag `sha-<7>` depuis le SHA de build · cloner `vermeer-gitops` · éditer **la seule ligne `tag:`** de `dev/llm/helm-release.yaml` et `staging/llm/helm-release.yaml` · pousser `bump/sha-*` (`--force` assumé : contenu reproductible, aucun autre auteur sur ces branches) · ouvrir la PR · **la merger en squash** (unique exception au « aucun merge automatique », bornée — [GOVERNANCE.md §3](GOVERNANCE.md#release-train-vermeer-text--vermeer-gitops)) · attendre Flux · dispatcher la QA staging · ouvrir/commenter l'issue `release-train` en cas d'échec |
