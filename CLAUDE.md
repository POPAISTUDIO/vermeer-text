...
Dernière passe : bouton jour/nuit dans le header (issue #141) — `FORCE_VERMEER_DARK` retiré de `App.jsx` au profit d'une garde « défaut sombre au premier passage seulement », flag `General.tsx` renommé `SHOW_THEME_DROPDOWN` (§7), `Header.tsx` et `App.jsx` inscrits en watchlist §11, décompte de la watchlist recompté (16 périmé → 24).

Passe précédente (2026-06-09) : mémoire-assistant partagée (approche B) mergée sur `main` + consolidation de la watchlist mémoire §11 (trois chantiers mémoire désormais mergés, fichiers natifs en watchlist active), décision étanchéité cross-BU de la mémoire partagée §5.3, transition de la propriété déploiement (départ progressif d'Oussama) §3.
