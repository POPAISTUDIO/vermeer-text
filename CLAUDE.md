...
Dernière passe : clôture documentaire du chantier 5 (standby). **Trois affirmations de ce fichier étaient fausses et sont corrigées** : §7 et §9 sur `balance`, décrit comme « laissé commenté en config » alors qu'il est **actif en staging ET en production**, ce qui rendait la limitation BudgetCard de §9 inexistante ; §8 dont le backlog V2 « auto-création Balance » est **de fait satisfait**. Watchlist §11 : `api/server/controllers/agents/request.js` inscrit (garde Vermeer « Message vide » non déclaré depuis sa pose), décompte 24 → **25**. Nouveaux documents de référence : [`docs/RUNBOOK-REVEIL.md`](docs/RUNBOOK-REVEIL.md), [`docs/RUNBOOK-MEP-MANUEL.md`](docs/RUNBOOK-MEP-MANUEL.md), [`docs/DETTES-OUVERTES.md`](docs/DETTES-OUVERTES.md).

Passe précédente (2026-07-30) : bouton jour/nuit dans le header (issue #141) — `FORCE_VERMEER_DARK` retiré de `App.jsx` au profit d'une garde « défaut sombre au premier passage seulement », flag `General.tsx` renommé `SHOW_THEME_DROPDOWN` (§7), `Header.tsx` et `App.jsx` inscrits en watchlist §11, décompte de la watchlist recompté (16 périmé → 24).

Passe antérieure (2026-06-09) : mémoire-assistant partagée (approche B) mergée sur `main` + consolidation de la watchlist mémoire §11 (trois chantiers mémoire désormais mergés, fichiers natifs en watchlist active), décision étanchéité cross-BU de la mémoire partagée §5.3, transition de la propriété déploiement (départ progressif d'Oussama) §3.
...
- **Design system Vermeer V1** : dark mode par défaut (défaut au premier passage, avec bascule jour/nuit dans le header depuis l'issue #141 ; le mode clair est le thème LibreChat brut, non brandé), accent rouge Vermeer `#E5384A` (hover `#C52838`), fond noir principal `#0A0A0B` (`--surface-primary`) avec surfaces anthracite `#141416` / `#1A1A1C`. Variables définies dans la section `.dark` de `client/src/style.css`. Valeurs V1 best-guess, à raffiner en atelier specs avec Antoine.
...
⚠️ **`balance` et `transactions` sont ACTIFS — corrigé le 31/07/2026.** Cette section affirmait qu'ils étaient « présents nativement mais laissés commentés en config » : **c'était faux**, et faux dans les deux environnements qui tournent. `balance.enabled: true` en **staging** (`vermeer-gitops`, `staging/llm/librechat.yaml:109`) **et** en **production** (`vermeer-gitops-prod`, `prod/llm/librechat.yaml:109`) — OBSERVED. Le credit management est donc branché partout où il tourne ; `transactions` suit par le forçage documenté en §11. Le plafond effectif est `DEFAULT_MONTHLY_BUDGET = 10_000_000` tokenCredits = **10 USD/mois**, et le reset mensuel est actif (`VERMEER_BUDGET_RESET_ENABLED` est un **kill-switch à défaut activé** : seul `'false'` désarme le scheduler). L'écart entre le yaml commité — où le bloc est bien commenté — et le déployé est tracé dans [`docs/CONFIG-DRIFT.md`](docs/CONFIG-DRIFT.md) §1, qui est la source de vérité sur ce point.

**Feature flags hardcodés — inventaire (audit CC)** : 10 noms distincts / 12 déclarations recensés à l'origine ; après le revert de `a98a8d6cd`, 9 noms / 11 déclarations restaient dans le code ; après le retrait de `FORCE_VERMEER_DARK` (issue #141, bouton jour/nuit), **9 noms / 10 déclarations**.
...
_Catégorie B — branding/cosmétique, sans équivalent natif → restent légitimement en code, à documenter. 5 noms / 5 déclarations :_
- ~~`FORCE_VERMEER_DARK` ×2 (`App.jsx`, `General.tsx`)~~ — **retiré (issue #141)**. `App.jsx` n'a plus de flag : le sombre reste le défaut mais via une garde « premier passage seulement » (cf. watchlist §11). La déclaration de `General.tsx` est renommée ci-dessous.
- `SHOW_THEME_DROPDOWN` (`Nav/SettingsTabs/General/General.tsx`) — masque le sélecteur de thème à 3 options du panneau Paramètres, qui exposerait un mode « Système » contredisant la bascule binaire du header. Flag autonome, ne dépend plus d'`App.jsx`.
...
- ~~**V2 — BudgetCard auto-création Balance**~~ — **SANS OBJET, constaté le 31/07/2026.** Le besoin supposait `balance` commenté en config. Il ne l'est pas (cf. §7) : avec `balance.enabled: true`, le document Balance **est créé sans aucune intervention admin**. OBSERVED sur un compte créé le 31/07/2026 en production — `GET /api/balance` renvoie un document complet (`monthlyBudget: 10000000`, `currentMonthSpend: 0`) sans qu'un seuil ait été édité. Rien à développer : ce backlog est **de fait satisfait**.
...
- ~~**BudgetCard user-facing — V1**~~ — **LIMITATION INEXISTANTE, corrigé le 31/07/2026.** Cette entrée décrivait un workaround (« un admin doit éditer un seuil pour le user, ce qui crée le doc Balance ») déduit de la prémisse « le bloc `balance` est commenté dans librechat.yaml ». La prémisse est fausse dans les environnements déployés (cf. §7), donc **le workaround n'a jamais eu lieu d'être** : le document Balance est créé sans intervention. Ne pas le mentionner en démo — c'est une limitation qui n'existe pas. Vérification de première main : `GET /api/balance` sur un compte neuf de production, 31/07/2026.
...
- **Code natif LibreChat modifié — watchlist merge upstream**. À chaque merge depuis upstream LibreChat, contrôler ces 25 fichiers en priorité, car nous y avons ajouté du code Vermeer susceptible de conflit. _(Décompte recompté le 2026-07-30 : le « 16 » annoncé jusque-là était périmé — 22 fichiers réellement listés, sur 21 puces dont une couvre `DeleteButton.tsx` + `DuplicateAgent.tsx` — plus `Header.tsx` et `App.jsx` inscrits par l'issue #141. **Recompté le 2026-07-31 : 24 → 25**, `api/server/controllers/agents/request.js` ayant été trouvé porteur d'un garde Vermeer sans être inscrit. Source de vérité automatisable : `.claude/scripts/watchlist.sh`.)_ :
  - `api/server/controllers/Balance.js` (enrichi avec currentMonthSpend dans la réponse /api/balance)
  - `api/server/controllers/agents/request.js` (**inscrit le 2026-07-31 — il portait un garde Vermeer non déclaré depuis sa pose**). Concern : **rejet des requêtes sans contenu utilisable**. Bloc `// Vermeer —` (l. ~103-112 du chemin `ResumableAgentController`) : si la requête n'a ni texte ni fichier (`!hasText && !hasFiles`), réponse `400 {"error":"Message vide : ajoutez du texte ou un fichier."}`. Motif : persister un message utilisateur vide produit un bloc de contenu texte vide que l'Anthropic API rejette en 400 « text content blocks must contain non-whitespace text », ce qui **empoisonne toute la conversation** ; les messages image-only restent valides, `hasFiles` les couvre. **Deux propriétés à connaître avant d'y toucher** : (a) le garde vit dans le contrôleur, donc **après** `checkAgentAccess` — ce qui en fait accessoirement une sonde de permission gratuite (403 si le rôle n'a pas `AGENTS.USE`, 400 « Message vide » s'il l'a, sans jamais atteindre un fournisseur), propriété utilisée par la sonde de production ; (b) le contrôleur exporté `AgentController` n'est qu'un wrapper de trois lignes qui délègue à `ResumableAgentController` — `_LegacyAgentController`, préfixé `_`, est mort. Ne pas retirer ce garde sans une validation équivalente en amont.
  ...
  - `client/src/components/Messages/Content/Error.tsx` (composant de rendu de la bulle d'erreur, **upstream modifié Vermeer**, fix QA n°12 wagon rc5). Concern : **mapping erreurs provider humanisé en aval du matching JSON `errorMessages`**. Point de convergence unique des 3 chemins d'erreur (content part `type:'error'` avalé dans le message FINAL, message-level `error:true`, frame `event:error`). Le composant garde le matching upstream par code structuré (`json.code || json.type` → table `errorMessages` → clés `com_error_*`) INTACT ; le mapping Vermeer (`mapProviderError` de `providerError.ts`) est branché **uniquement** en remplacement des deux `return defaultResponse` (chemins `!isJson` et `else` de fallback), donc en aval du matching JSON — il ne s'exécute QUE si aucun code structuré n'a matché. Rendu : `<ProviderError>` (message humain localisé + ligne « Détail technique » repliée via `<details>`). Au merge upstream, **vérifier que cette branche de fallback n'est pas court-circuitée si la table `errorMessages` / la logique de matching est restructurée** (ex. upstream qui ajouterait son propre fallback avant nos deux points de branchement écraserait le mapping provider). Ne pas déplacer le mapping avant le matching JSON (écraserait `token_balance`/`moderation`/keys/`google_tool_conflict`…). Spec de contrat : `providerError.spec.ts` (14 cas).
  - _Helper Vermeer (nouveau fichier, pas du code natif — pas de risque de conflit merge, hors décompte) : `client/src/components/Messages/Content/providerError.ts` (`mapProviderError(text): ProviderErrorKey | null` : pattern-matche la string d'erreur SDK — format axios stable « ...failed with status code NNN », tolère la forme préfixée serveur — vers 6 familles `400 / 401|403 / 404 / 429 / 5xx|overloaded|529 / timeout`, `null` sinon. Le status HTTP n'existe que côté serveur / Loki #50 ; le client ne reçoit qu'une string → pattern-matching assumé. Consommé par `Error.tsx`, testé par `providerError.spec.ts`)._
  - `client/src/components/Agents/AgentCard.tsx` (carte de la grille Assistants/Marketplace, **upstream modifié Vermeer**). Concerns : (a) **fermeture du détail avant ouverture builder (`3b8f516b7`)** — `AgentDetailContent onRequestClose={() => setIsOpen(false)}` évite l'empilement de modales ; (b) **badge de catégorie remis dans le flux (fix QA n°13)** — le layout externe passe de `flex ... gap-5` (rangée avatar+texte) à `flex-col` : une ligne d'en-tête `flex justify-end` porte le badge (`max-w-full truncate`), suivie d'une rangée `flex min-h-0 flex-1 gap-5` [avatar `self-center` | contenu `justify-center`]. Origine : le badge upstream était en `absolute right-4 top-3` et recouvrait la 1re ligne du nom/description pour les libellés FR longs de la taxonomie v2 (`com_agents_category_*`, emoji inclus — ex. « ✍️ Conception & écriture », « 💡 Expertises digitales »). Un padding-right fixe (option écartée) aurait dû réserver ~160px et amputer la moitié du texte. En flux, le texte flotte toujours **sous** le badge quelle que soit sa longueur ; hauteur de carte inchangée (`h-32 md:h-36 lg:h-40` + `overflow-hidden`, grille `grid-cols-1 md:grid-cols-2` uniforme). Les tests `tests/AgentCard.spec.tsx` n'asservissent que le texte du libellé, pas la position CSS. Ne pas re-passer le badge en `absolute` sans réserver l'espace du pire cas de libellé.
  - `client/src/components/Conversations/Convo.tsx` (item de conversation de la sidebar, **upstream modifié Vermeer**). Concern : **raccourci « Shift = actions rapides » désactivé (fix QA n°15)** — la prop passée à `ConvoOptions` est forcée `isShiftHeld: false` (au lieu de `isActiveConvo ? isShiftHeld : false`), neutralisant la branche upstream `ConvoOptions.tsx:337-366` qui, sur la conversation active + Shift maintenu, remplaçait le menu ⋯ par Archiver + une **suppression instantanée SANS confirmation**. Origine : remonté en QA comme un « menu incomplet après duplication » (faux positif — c'était ce raccourci, pas un bug de duplication ; l'objet dupliqué est prouvé complet via `getConvo().lean()`, et aucun champ de la conversation ne gouverne le menu). Décision produit : dangereux et illisible pour des profils métier. **Neutralisation, pas suppression** : le bloc `ConvoOptions.tsx:337-366` reste intact (merge upstream sans conflit, réactivable). Le hook `useShiftKey()` (l.40) est **conservé** car encore consommé par la largeur CSS `max-w-[60px]/[28px]` (l.~283) — au merge upstream, vérifier que la neutralisation tient si la signature de `convoOptionsProps`/`isShiftHeld` change côté amont. FYI cosmétique connu (non corrigé, hors scope) : maintenir Shift sur la conversation active élargit encore le slot à 60px pour n'afficher que le ⋯ (léger espace vide) — inoffensif, réservé à une interaction Shift délibérée. Ne pas retirer le force-`false` sans réintroduire une confirmation sur la suppression rapide.
  - `client/src/components/UnifiedSidebar/SectionModal.tsx` (wrapper de modale générique Vermeer — Assistants/Skills/Paramètres/Fichiers/Mémoires + builder ; **nouveau fichier Vermeer, mais enrobe des primitives Radix upstream `OGDialog`**). Concern : **prop `dismissOnOutside?: boolean` (défaut `true`) — modale builder persistante (fix QA n°16)**. En `nonModal` (compromis z-index pour rendre cliquables les popovers Ariakit portalisés), Radix ferme la modale à TOUTE interaction extérieure ; le garde `keepOpenOnPopover` ne neutralisait que les popovers → clic sur « fermer la barre » (desktop) / backdrop + `inert` du drawer (mobile) fermaient la modale et **détruisaient la saisie** (le builder n'est pas démonté par le collapse — `MonoSidebar`/`UnifiedSidebar` restent montés, `Root.tsx:75` inconditionnel, modale portalisée vers `body` — c'est bien le light-dismiss nonModal, pas un unmount). `dismissOnOutside={false}` fait `event.preventDefault()` **inconditionnel en tête** des 3 handlers `onPointerDownOutside`/`onFocusOutside`/`onInteractOutside` (via `keepOpenOnPopover`, seulement branchés sous `nonModal`), **en amont** du garde popover existant (non court-circuité → Catégorie/Fournisseur/Modèle toujours OK). Posé sur le **builderModal SEUL** (`MonoSidebar.tsx:91-101`) ; `sectionModal` garde son light-dismiss (pas de saisie longue). Sorties restantes : bouton **X** (`showCloseButton` défaut `true`), **Escape**, et les fermetures programmatiques `setOpenBuilder(null)` (create/update/delete/nav, invariants #49/#55/#59/n°9 inchangés — elles ne dépendent pas du dismiss-outside). Au merge upstream, vérifier que les 3 handlers Radix (`DismissableLayer`) et la signature `OGDialogContent` tiennent ; ne pas retirer le prop sans réintroduire une persistance équivalente de la saisie builder.
  - `client/src/components/Chat/Header.tsx` (en-tête de la vue de conversation, **upstream modifié Vermeer**). Concerns : (a) **`SHOW_PRESETS_BUTTON = false`** et **`SHOW_COMPARISON_BUTTON = false`** — masquages UI (catégorie B §7), `PresetsMenu` et la logique multi-convo préservés ; (b) **bascule jour/nuit montée dans le header (issue #141)** — composant local `ThemeToggle` = `<TooltipAnchor description={localize('com_ui_toggle_theme')} side="bottom">` enrobant `<ThemeSelector returnThemeOnly />` de `@librechat/client`, monté à la fin des DEUX groupes d'items de droite : la zone desktop (`{!isSmallScreen && <div className="flex items-center gap-2">}`) et le groupe small-screen replié dans le cluster de gauche — même schéma que `ExportAndShareMenu`/`TemporaryChat`, qui migrent d'un groupe à l'autre selon `useMediaQuery('(max-width: 768px)')`. `returnThemeOnly` donne la bascule **binaire** (`nextTheme = isDark(theme) ? 'light' : 'dark'`), l'`aria-label` localisé et l'anti-rebond 500 ms d'upstream : aucun mode « Système » n'est atteignable par ce bouton, contrairement au `ThemeSelector` local à 3 options de `General.tsx` (masqué par `SHOW_THEME_DROPDOWN`, consommé aussi par `ShareView.tsx`). Ne pas remplacer par un composant maison (on perdrait l'a11y et l'anti-rebond amont), et ne pas retirer un des deux points de montage sans acter que la bascule disparaît sur ce format d'écran. Le défaut « sombre » ne vit PAS ici mais dans `App.jsx` (ci-dessous).
  - `client/src/App.jsx` (racine de la SPA, **upstream modifié Vermeer**). Concern : **défaut sombre « au premier passage seulement » (issue #141)** — `getThemeProviderProps(envTheme)` ne passe `initialTheme: 'dark'` au `ThemeProvider` que si `localStorage` n'a **aucune** valeur sous `THEME_STORAGE_KEY = 'color-theme'` (la `THEME_KEY` de `packages/client/.../ThemeProvider.tsx`) ; si une préférence est stockée, **aucune** prop de thème n'est passée et `getInitialTheme()` la relit. Pourquoi la solution évidente ne marche pas : le `useEffect` d'init du `ThemeProvider` est gardé par `initialized.current` mais l'App remonte à chaque chargement de page, donc un `initialTheme` en dur est réappliqué ET réécrit dans `localStorage` à chaque F5 — le choix « clair » de l'utilisateur ne survivrait jamais à un rechargement. Sans garde du tout, le défaut natif retomberait sur `'system'` (`getInitialTheme()`), pas sur `'dark'` : les deux extrêmes sont faux, la garde est le seul état correct. La branche `envTheme` (`REACT_APP_THEME_*`, `getThemeFromEnv`) garde la priorité et le comportement upstream `{ initialTheme: 'system', themeRGB }`. **Ne pas modifier `ThemeProvider.tsx`** pour ce besoin : la garde vit ici. Ne pas repasser `initialTheme: 'dark'` en dur sans réintroduire une persistance équivalente du choix utilisateur.
...
- **Patch TEMPORAIRE `@langchain/core@1.1.44` — exports CJS du uuid vendored cassés (`(0, _langchain_core_utils_uuid.v4) is not a function`).** Via `patch-package` (`patches/@langchain+core+1.1.44.patch`), appliqué au même `postinstall`. Branche `fix/issue-128-uuid`, issue #128, candidate `v0.10.23`.
  - **Symptôme** : crash **intermittent** sur le chemin assistants/RAG, `(0, _langchain_core_utils_uuid.v4) is not a function` remonté dans `#sendCompletion` (`api/server/controllers/agents/client.js`), suivi d'une *Unhandled promise rejection*. 3 occurrences en staging le 2026-07-28.
  - **Cause amont** : bug d'emit CJS (rolldown) dans `dist/utils/uuid/index.cjs`. Les réexports de fin de fichier font `exports.v4 = require_v4` — c'est-à-dire le **namespace de module** `{ default: fn }` — au lieu de `require_v4.default`, alors que la map `uuid_exports` juste au-dessus utilise correctement `.default`. **Les 10 exports** sont touchés (`v1/v4/v5/v7/parse/stringify/validate/version/MAX/NIL`). Le build **ESM (`index.js`) est sain** ; `/api` étant CJS, il résout le `.cjs` cassé. Les types `.d.cts` déclarent bien des fonctions → **TS ne peut pas détecter ce décalage types/runtime**.
  - **Pourquoi intermittent** : les seuls consommateurs de la sous-voie `@langchain/core/utils/uuid` sont **Google/Gemini et Mistral** (jamais Anthropic/OpenAI) — `google-genai/dist/utils/common.cjs:285,369`, `google-common/dist/utils/gemini.cjs:484`, `google-common/dist/experimental/utils/media_core.cjs:171-172`. Aux trois premiers, `v4()` n'est appelé **que si le modèle renvoie un `functionCall` sans champ `id`** : pas de tool call ou tool call avec id → aucun crash. D'où le profil intermittent, borné au provider Google avec outils bindés.
  - **⚠️ La prod v0.10.10 n'est pas saine, elle est non exercée** : elle embarque le même `@langchain/core@1.1.44` cassé (lockfile **byte-identique**, même integrity hash). Ne pas conclure d'essais propres que la prod est immune — il faut router sur Gemini avec tool call pour déclencher.
  - **Correctif** : `exports.X = require_X.default;` sur les 10 réexports de `dist/utils/uuid/index.cjs` (aligné sur ce que fait déjà le build ESM). `uuid_exports` **laissé intact** (déjà correct). Un `overrides` npm ne peut PAS corriger ce bug : il est dans le code vendored de `@langchain/core`, pas dans un conflit de version du package npm `uuid` (lui sain — `uuid@11.1.1`, `v4` bien exporté, aucun doublon de `@langchain/core` : tout est `deduped`).
  - **Câblage** : réutilise le `postinstall: patch-package` et le `COPY patches` du Dockerfile déjà en place (rien à ajouter). `package.json`/`package-lock.json` **inchangés** (aucune dep touchée, rien à régénérer).
  - **⚠️ TEMPORAIRE — épinglé à `@langchain/core@1.1.44`** : tout bump déplace les lignes ciblées et fait échouer `patch-package` (warning au postinstall). `@langchain/core` est une dep **transitive** de `@librechat/agents` → un bump de ce dernier peut la déplacer sans changement explicite côté `package.json`. Revalider/régénérer après tout merge upstream touchant `@langchain/core` ou `@librechat/agents`.
  - **Chantier de suivi** : au bump `@librechat/agents` → **3.2.35** (sous Node 24), vérifier la version de `@langchain/core` résolue et si l'emit CJS est corrigé en amont avant de retirer ce patch. Test de non-régression rapide : `node -e "console.log(typeof require('@langchain/core/utils/uuid').v4)"` doit afficher `function`.
